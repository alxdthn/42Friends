...
class RotateView @JvmOverloads constructor(
	...
	private val layout = R.layout.user_item
	...
	private fun moveView() {
		if (payload[0].translationX == payload[1].translationX) {
			payload[0].animateTranslation(500, AnimType.SHOW_RIGHT).subscribe()
		} else {
			Observable.fromIterable(payload).subscribe { view ->
				if (view.translationX == 0f) {
					view.animateTranslation(500, AnimType.HIDE_LEFT)
						.subscribe {
							view.translationX = winWidth
						}
				} else {
					view.animateTranslation(500, AnimType.SHOW_RIGHT).subscribe()
				}
			}.addTo(composite)
		}
	}
