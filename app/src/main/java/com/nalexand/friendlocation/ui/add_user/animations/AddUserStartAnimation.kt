...
class AddUserStartAnimation(private val composite: CompositeDisposable) {
	...
	fun start(group: ViewGroup) {
		group.apply {
			cvAddUserHeader.animatePos(250, AnimType.SHOW_TOP)
				.mergeWith(cvAddUserInput.animatePos(500, AnimType.SHOW_TOP))
				.mergeWith(flAddUserButton.animatePos(1000, AnimType.SHOW_TOP))
